...
export function generateStl(appState, domElements) {
    ...
    const { 
        layerHeightInput, xSizeInput, ySizeInput, 
        bandThicknessInput, baseThicknessInput, numBandsInput 
    } = domElements;
    ...
    for (let i = 0; i < imageWidth - 1; i++) {
        ...
        faces.push([bottomVertices[jf * imageWidth + i], topVertices[jf * imageWidth + i], topVertices[jf * imageWidth + i + 1]]);
        faces.push([bottomVertices[jf * imageWidth + i], topVertices[jf * imageWidth + i + 1], bottomVertices[jf * imageWidth + i + 1]]);
        
        // Back edge faces (connecting bottom to top)
        faces.push([bottomVertices[jb * imageWidth + i], topVertices[jb * imageWidth + i + 1], topVertices[jb * imageWidth + i]]);
        faces.push([bottomVertices[jb * imageWidth + i], bottomVertices[jb * imageWidth + i + 1], topVertices[jb * imageWidth + i + 1]]);
    }
    ...
    for (let j = 0; j < imageHeight - 1; j++) {
        ...
        faces.push([bottomVertices[j * imageWidth + il], topVertices[j * imageWidth + il], topVertices[(j + 1) * imageWidth + il]]);
        faces.push([bottomVertices[j * imageWidth + il], topVertices[(j + 1) * imageWidth + il], bottomVertices[(j + 1) * imageWidth + il]]);
        
        // Right edge faces (connecting bottom to top)
        faces.push([bottomVertices[j * imageWidth + ir], topVertices[(j + 1) * imageWidth + ir], topVertices[j * imageWidth + ir]]);
        faces.push([bottomVertices[j * imageWidth + ir], bottomVertices[(j + 1) * imageWidth + ir], topVertices[(j + 1) * imageWidth + ir]]);
    }
    ...
    for (const face of faces) {
        ...
        const ux = v2[0] - v1[0], uy = v2[1] - v1[1], uz = v2[2] - v1[2]; // Edge vector 1
        const vx = v3[0] - v1[0], vy = v3[1] - v1[1], vz = v3[2] - v1[2]; // Edge vector 2
        ...
        for (const v of [v1, v2, v3]) {
            writer.setFloat32(offset, v[0], true); offset += 4; // X
            writer.setFloat32(offset, v[1], true); offset += 4; // Y
            writer.setFloat32(offset, v[2], true); offset += 4; // Z
        }
